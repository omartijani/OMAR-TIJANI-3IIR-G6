...
let rand=[];
    ...
    function gen(){ const ra= courses[(Math.floor(Math.random() * (courses.length)))];
         if (!rand.includes(ra)) {
            rand.push(ra);
            return ra;
         } 
    ...
    const uniqueNumber = (courses) => {
        for(let i=0;i<3;i++){gen()}}
    uniqueNumber(courses);
rand.forEach((f) => {
 
div.innerHTML+="<div class='image'> <img src='"+f.image+"'><p class='title'>"+f.title+"</p><p class='price'>"+f.price+" </p></div>";
